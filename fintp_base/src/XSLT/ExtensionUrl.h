...
namespace FinTP
{
	class ExportedObject FunctionUrl : public Function
	{
		...
		public :
			...
			static string m_BAAuthentication;
		...
		private:
			...
			void initCurl( CURL *&conn, const string& url, curl_slist* headerList = NULL ) const;

			std::unique_ptr<XERCES_CPP_NAMESPACE_QUALIFIER DOMDocument> createErrorXml( const string& url, const string& errorMessage = "", const string& errorStatus = "" ) const;

			static char m_ErrorBuffer[ CURL_ERROR_SIZE ];
			static vector<unsigned char> m_Buffer;
			static XercesDocumentWrapper* m_XercesDocumentWrapper;
	};
}
