...
string XPathHelper::Evaluate( const string& xPath, string& doc, const string& defaultPrefix )
{
	XALAN_USING_XALAN( XercesDocumentWrapper );
	XALAN_USING_XALAN( XalanDocument );
	XALAN_USING_XALAN( XalanNode);

	XercesDocumentWrapper *documentWrapper;
	XalanDocument *xalanDocument;
	XalanNode* returnNode;
	string returnString = "";
#ifdef XALAN_1_9
	XALAN_USING_XERCES( XMLPlatformUtils )
	documentWrapper = new XercesDocumentWrapper( *XMLPlatformUtils::fgMemoryManager, XmlUtil::DeserializeFromString( doc ), true, true, true );
#else
	documentWrapper = new XercesDocumentWrapper( m_Document, true, true, true );
#endif
	xalanDocument = ( XalanDocument* )documentWrapper;

	if (xalanDocument == NULL)
		throw runtime_error("Unable to parse payload.");

	try
	{
		returnNode = Evaluate( xPath, xalanDocument, defaultPrefix );
		returnString = XPathHelper::SerializeToString( returnNode );
	}
	catch( ... )
		TRACE ("Unable to Evaluate. XPath=[" << xPath << "] , Document=[" << doc << "]");

	if( documentWrapper != NULL )
	{
		delete documentWrapper->getXercesDocument();
		delete documentWrapper;
