...
AppSettings::AppSettings( const string& configFilename, const string& configContent, const string& encryptionKey )
{
	try
	{
		loadConfig( configFilename, configContent, encryptionKey );
	}
	catch( ... )
	{
...
void AppSettings::loadConfig( const string& configFilename, const string& configContent, const string& encryptionKey )
{
	...
	try
	{
		bool isEncrypted = Base64::isBase64( configContent ) && !encryptionKey.empty();
		if ( configContent.length() > 0 )
		{
			if ( isEncrypted )
			{
				DEBUG( "Config appears to be encrypted" );
				if ( encryptionKey.empty() )
					throw runtime_error( "Config is encrypted but no key was supplied!" );

				const string cipherText = Base64::decode( configContent );

				AES cipher( reinterpret_cast<const unsigned char*>(encryptionKey.c_str()), encryptionKey.size() );

				const unsigned char* cipherTextC_str = reinterpret_cast<const unsigned char*>( cipherText.c_str( ) );
				const vector<unsigned char> decryptedConfig = cipher.AES_decrypt( cipherTextC_str, cipherText.size() );

				parseDocument = XmlUtil::DeserializeFromString( string( decryptedConfig.begin(), decryptedConfig.end() ) );
				root = parseDocument->getDocumentElement();
				DEBUG( "Config successfully decrypted" );
			}
			else
