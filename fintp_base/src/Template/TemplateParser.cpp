	...
	m_Regex( false ), m_MatchEnd( 0 ), m_Parent( NULL )
{
	...
	if ( node->getNodeType() != DOMNode::ELEMENT_NODE )
	{
		...
		errorMessage << "Parameter [node] passed to TemplateData is not an [ELEMENT_NODE], but [" << node->getNodeType() << "]";
...
TemplateData::NodeMatchReturn TemplateData::MatchSingle( const ManagedBuffer& buffer, DOMElement *root )
{
	string bufferEx = "";
	if ( buffer.size() > 10 )
	...
	catch ( const TranscodingException& ex )
	{
		stringstream errorMessage;
		errorMessage << "Match single failure : TranscodingException type : " << ( int )ex.getErrorType() <<
			" Code : [" << ex.getCode() << "] Message = " << localForm( ex.getMessage() ) <<
			" Line : " << ex.getSrcLine();
		TRACE( errorMessage.str() );
		throw runtime_error( errorMessage.str() );
	}
	...
	{
		stringstream errorMessage;
		errorMessage << "Match single failure : Type : " << ( int )ex.getErrorType() <<
			" Code : [" << ex.getCode() << "] Message = " << localForm( ex.getMessage() ) <<
			" Line : " << ex.getSrcLine();
		TRACE( errorMessage.str() );
		throw runtime_error( errorMessage.str() );
	...
	{
		stringstream errorMessage;
		errorMessage << "Match single failure :" << ex.what();
		TRACE( errorMessage.str() );
		throw;
	}
	catch( ... )
...
TemplateData::NodeMatchReturn TemplateData::Matches( const ManagedBuffer& buff, DOMNode *root )
{
	if ( m_Regex )	
	{
		//DEBUG( "REGEX" );
		ManagedBuffer buffer( buff.buffer(), ManagedBuffer::Ref, buff.size() );
...
*/
bool TemplateNode::MatchTemplate( const ManagedBuffer& buff, DOMNode *root )
{
...
bool TemplateNodeRoot::MatchTemplate( const ManagedBuffer& buffer, DOMNode *root )
{
	...
	bool childMatch;
	if ( buff.size() == 0 )
		childMatch = m_Children[ 0 ].MatchTemplate( buffer, element );
	else
		childMatch = m_Children[ 0 ].MatchTemplate( buff, element );
...
*/
bool TemplateParser::Matches( const ManagedBuffer& buffer, XERCES_CPP_NAMESPACE_QUALIFIER DOMNode *root )
{
	// guard incoming params
	if ( root == NULL )
	{
