...
class ExportedObject InternalEvaluatorWrapper
{
	...
	private :
		static const string m_FieldNames[ ENRICHEVALUATORLWRAPPER_FIELDCOUNT ];
...
class ExportedObject InternalXmlPayload
{
	public : 

		/**
		 * Questioned values are only used by Enrich feature
		 * Fields used by Routing Evaluators should be replaced by KEYWORD usage
		 */
		enum Fields
			{
				SENDER = 0,
...
class ExportedObject RoutingMessageBase
{
	public:

		virtual void setBatchId( const string& batchId ) = 0;
		virtual void setBatchSequence( const unsigned long sequence ) = 0 ;
		virtual void setBatchTotalCount( const unsigned long batchCount ) = 0;
		virtual void setGenerateId() = 0;
};
...
class ExportedObject RoutingMessageEvaluator 
{
	...
	protected :
		...
		**/
		...
		/**
		 * Specific keywords definitions of current message type
		**/		
		static RoutingKeywordCollection* m_Keywords;

		/**   
		 * Specific keyword mappings of current message type
		**/
		map< string, string > m_KeywordMappings;

		/**
		 * Selector type to be consider for the current message  type.
		**/
	...
	protected : 
		...
		virtual string internalGetField( const string& field ) = 0;
		...
		/**
		 * By convention the message type is the first child element of the root.
		 */
	...
	private:
		...
		// Callbacks to overcome database calls from plugins: AchBlkRjctPayload, AchBlkAccPayload, AchBlkRjctPayload, AchCoreBlkDDbtRflPayload, DiCoreBlkRflPayload
	...
	public:
		...
		/**
		 * Gets keyword names, defined by database config and evaluator overridden
		 */

		virtual const vector<string> getKeywordNames(); 
		static void setKeywords( const RoutingKeywordCollection& keywords );
		void setIsoType( const bool isoMessageType ) { m_IsoMessageType = isoMessageType; }
		...
		virtual NameValueCollection getAddParams( const string& ref )
		{
		...
		virtual void UpdateMessage( RoutingMessageBase* message ){};
