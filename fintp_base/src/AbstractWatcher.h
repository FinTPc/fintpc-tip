...
namespace FinTP
{
	class ExportedObject AbstractWatcher
	{
		...
		protected : 
			...
			virtual void Init(const AbstractFilter& filter) {}

		public:
			
			virtual ~AbstractWatcher();		

			virtual void setMessageThrottling( unsigned int throttling ) {}

			static AbstractWatcher* createWatcher( const AbstractFilter& filter, NotificationPool* notificationPool );
