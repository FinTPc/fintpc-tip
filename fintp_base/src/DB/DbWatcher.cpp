...
void DbWatcher::Init(const AbstractFilter& filter)
{
	const DbFilter& dbFilter = dynamic_cast<const DbFilter&>(filter);

	setProvider( dbFilter.getProvider() );
	setSelectSPName( dbFilter.getSelectSPName() );
	setConnectionString( dbFilter.getConnectionString() );
	setFullObjectNotification( true );
}