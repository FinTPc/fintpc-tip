...
AbstractWatcher* AbstractWatcher::createWatcher( const AbstractFilter& filter, NotificationPool* notificationPool )
{
	const FilterType::FilterTypeEnum filterType = filter.getFilterType();
	unique_ptr<AbstractWatcher> returnedWatcher;
	switch ( filterType )
	{
		case FilterType::MQ:
		{
			returnedWatcher.reset( new MqWatcher( notificationPool ) );
			break;
		}
		case FilterType::DB:
		{
			returnedWatcher.reset( new DbWatcher( notificationPool ) );
			break;
		}
		default:
			throw runtime_error( "Invalid Watcher type" );
	}
	returnedWatcher->Init( filter );
	return returnedWatcher.release();
}