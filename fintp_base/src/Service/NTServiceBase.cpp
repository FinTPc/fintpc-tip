...
// last revised: $Date: 2013-11-13 18:29:44 +0200 (Wed, 13 Nov 2013) $, $Revision: 21508 $
...
CNTService * FinTP::AfxGetService() { return gpTheService; }
...
, m_pszPassword(0)
{
	TRACE_SERVICE( "Creating service instance ..." );

	::ZeroMemory( m_lpServiceName, sizeof( m_lpServiceName ) );
	::ZeroMemory( m_lpDisplayName, sizeof( m_lpDisplayName ) );

#ifdef CRT_SECURE

	_tcscpy_s( m_lpServiceName, sizeof( m_lpServiceName ), lpServiceName );
	if ( lpDisplayName ) 
		_tcscpy_s( m_lpDisplayName, sizeof( m_lpDisplayName ), lpDisplayName );
	else
		_tcscpy_s( m_lpDisplayName, sizeof( m_lpDisplayName ), lpServiceName );

#else
	_tcscpy( m_lpServiceName, lpServiceName );
	if ( lpDisplayName ) 
		_tcscpy( m_lpDisplayName, lpDisplayName );
	else
		_tcscpy( m_lpDisplayName, lpServiceName );
#endif
...
BOOL CNTService :: RegisterService( int argc, char ** argv ) 
{
	...
	TRACE_SERVICE( "RegisterService invoked ..." );
...
BOOL CNTService :: StartDispatcher() 
{
	...
	TRACE_SERVICE( "Starting dispatcher ..." );
...
BOOL CNTService :: InstallService()
{
	...
	{
		if ( strstr( m_argv[ i ], "--key=" ) )
		{
			string configEncryptionKey = string( m_argv[ i ] + 6, strlen( m_argv[ i ] ) - 6 );
			_tcscat( szPath, " --service_key=" );
			memset( m_argv[ i ] + 6, 'x', strlen( m_argv[ i ] ) - 6 );
		
			DATA_BLOB DataIn;
			DATA_BLOB DataOut;
			DWORD cbDataInput = configEncryptionKey.size();
			DataIn.pbData = (BYTE *)configEncryptionKey.c_str();
			...
			string key = Base64::encode( DataOut.pbData, DataOut.cbData );

			LocalFree(DataOut.pbData);

			_tcscat( szPath, key.c_str() );
...
LRESULT CALLBACK _FacelessWndProc_( HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam ) 
{
	if (uMsg==WM_QUERYENDSESSION || uMsg==WM_ENDSESSION || uMsg==WM_QUIT) 
	{
		if (lParam==NULL || uMsg==WM_QUIT)
		{
			...
			if (FinTP::AfxGetService()!=NULL)
				FinTP::AfxGetService()->Stop();	// stop me.
			return TRUE;
		}
	}
...
void WINAPI CNTService :: ServiceMain(DWORD dwArgc, LPTSTR *lpszArgv) 
{
	...
	TRACE_SERVICE( "Service main invoked..." );
	...
	if( ( gpTheService->m_sshStatusHandle ) && ( gpTheService->m_bAssumeRunBlocks ) )
...
#ifdef CRT_SECURE
	_tcscpy_s(szKey, 255, gszAppRegKey );
	_tcscat_s(szKey, 255 - _tcslen(gszAppRegKey), m_lpServiceName);
#else
		...
		if( lRet == ERROR_SUCCESS )
		{
			...
			if( lRet == ERROR_SUCCESS ) 
			{
				...
				if( ! * p ) 
				{
					...
					// too.
#ifdef CRT_SECURE
					_tcscpy_s(p, dwNewSize - 1, m_lpServiceName);
#else
		...
		if( lRet == ERROR_SUCCESS ) 
		{
			...
			if( lRet == ERROR_SUCCESS ) 
			{
				...
				for(; *p; p += _tcslen(p)+1)
				{
					...
					{
#ifdef CRT_SECURE
						_tcscpy_s(pNew, _tcslen(pNew) - 1, p);
#else
						_tcscpy(pNew, p);
#endif
						pNew += _tcslen(pNew)+1;
