...
MqWatcher::MqWatcher( NotificationPool* notificationPool, const string& queue, const string& queueManager, const string& transportURI, const string& connectionString )
	: InstrumentedObject(), AbstractWatcher( notificationPool ), m_WatchOptions( MqWatcher::NotifyMessage ), m_Queue( queue ), m_Throttling( 0 ),
...
void MqWatcher::Init(const AbstractFilter& filter)
{
	const MqFilter& mqFilter = dynamic_cast<const MqFilter&>( filter );

	if( mqFilter.isBatch() )
		setWatchOptions( MqWatcher::NotifyGroups | MqWatcher::NotifyUnique );

	setHelperType( mqFilter.getHelperType() );
	setQueue( mqFilter.getQueueName() );
	setQueueManager( mqFilter.getQueueManagerName() );
	setTransportURI( mqFilter.getTransportURI() );

	DEBUG( "Watch queue [" << m_Queue << "] on queue manager [" << m_QueueManager << "] using channel definition [" << m_TransportURI << "]" );
}