...
{
	std::unique_ptr<XERCES_CPP_NAMESPACE_QUALIFIER DOMDocument> inputDOM;
	...
	for( unsigned int i=0; i<m_Filters.size(); i++ )
	{
		...
		{
			case AbstractFilter::XmlToXml :
				crtFilter->ProcessMessage( inputDOM.get(), transportHeaders, asClient );
				break;
			case AbstractFilter::XmlToBuffer :
				crtFilter->ProcessMessage( inputDOM.get(), inputBuffer, transportHeaders, asClient );

				if ( crtFilter->getFilterType() == FilterType::MQ && i == m_Filters.size() - 1 )
				{
					if( outputData.get() != NULL )
					{
						outputData.get()->copyFrom( XmlUtil::SerializeToString( inputDOM.get() ) );
						DEBUG( "Done. Output data size : [" << outputData.get()->size() << "]" );
						return AbstractFilter::Completed;
					}
				}
