...
class SwiftXmlPlugin: public Plugin
{
public:
	SwiftXmlPlugin( const std::string& name, const std::string& version ): Plugin( name, version ) {}
	bool hasNamespace( const string& aNamespace )
	{
		return ( aNamespace == "urn:xmlns:SWIFTMTs" );
	}
	RoutingMessageEvaluator* newInstance( const XERCES_CPP_NAMESPACE_QUALIFIER DOMDocument* document )
	{
		return new SwiftXmlPayload( document );
	}
};
...
#endif
{
	return new SwiftXmlPlugin( "SwiftXml", "0.1" );
}
}
