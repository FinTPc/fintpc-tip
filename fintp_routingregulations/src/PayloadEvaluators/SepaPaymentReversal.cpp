...
SepaPaymentReversal::SepaPaymentReversal( XERCES_CPP_NAMESPACE_QUALIFIER DOMDocument* document, const string& messageFilename ) :
	MessageEvaluator( document, MessageEvaluator::Sepa_CreditTransfer, messageFilename )
{
	m_SequenceResponse = NULL;
}

SepaPaymentReversal::~SepaPaymentReversal()
{
	try
	{
...
bool SepaPaymentReversal::Validate()
{
	try
	{
		...
		DOMNodeList* msgElemNodes = rootElem->getElementsByTagName( unicodeFormWs( "pacs.007.001.01" ) );
		if ( ( msgElemNodes == NULL ) || ( msgElemNodes->getLength() == 0 ) )
			return false;
		
		DOMElement *msgElemNode = dynamic_cast< DOMElement* >( msgElemNodes->item( 0 ) );
		if ( msgElemNode == NULL )
			throw logic_error( "Expected [pacs.007.001.01] to be an element." );
		...
		if ( grpHdrNode == NULL )
			throw logic_error( "Expected [pacs.007.001.01/GrpHdr] to be an element." );
		...
		if ( hdrAmtNode == NULL )
			throw logic_error( "Expected [pacs.007.001.01/GrpHdr/TtlIntrBkSttlmAmt] to be an element." );
		...
		for ( unsigned int i=0; i<txInfNodes->getLength(); i++ )
		{
			...
			if ( txInfNode == NULL )
				throw logic_error( "Expected [pacs.007.001.01/CdtTrfTxInf] to be an element." );
			...
			if ( txAmtNode == NULL )
				throw logic_error( "Expected [pacs.007.001.01/CdtTrfTxInf/IntrBkSttlmAmt] to be an element." );
			...
			if ( strdRmtInfo.length() > 141 )
			{
				...
				if ( pmtIdNode == NULL )
					throw logic_error( "Expected [pacs.007.001.01/CdtTrfTxInf/PmtId] to be an element." );
				...
				if ( txIdNode == NULL )
					throw logic_error( "Expected [pacs.007.001.01/CdtTrfTxInf/PmtId/TxId] to be an element." );
